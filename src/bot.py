...
from rules import (get_rules, add_rule, delete_rule,
                   load_pending_rules, save_pending_rules, add_pending_rule)
from ocr   import (normalize_photo, normalize_text, normalize_pdf,
                   save_ocr_correction, load_ocr_corrections, parse_caption_brands)
from search import (find_items, keyword_search, smart_search, build_qa,
                    BRAND_TOKENS, claude_pick_batch)
from excel_builder import create_excel, parse_qty
from logger import (log_usage, get_usage_stats, log_not_found, get_catalog_gaps)
...
def _push_outlets(п):   # визначає скільки трубних виходів має PUSH-фітинг (коліно=2, трійник=3 тощо) для розрахунку гільз
    from search import TYPE_SYNONYMS
    qa  = п.get('_qa') or build_qa(п)
    п['_qa'] = qa
    typ = qa.get('type')
