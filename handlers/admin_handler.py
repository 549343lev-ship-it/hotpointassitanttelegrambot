...
def register(bot, state: dict):
    ...
    @bot.message_handler(commands=['synonyms_full_rebuild'])
    def cmd_synonyms_full_rebuild(message):
        if message.chat.id != ADMIN_ID: return
        status = bot.reply_to(message, "⏳ Повний rebuild: кеш + каталог (~20 сек)...")
        try:
            import threading
            def _run():
                from engine.synonyms import full_rebuild
                r = full_rebuild()
                bot.edit_message_text(
                    f"✅ *Повний rebuild завершено*\n"
                    f"Органічних ключів (з кешу): {r['organic_keys']}\n"
                    f"Органічних варіантів: {r['organic_entries']}\n"
                    f"Додано каталожних аналогів: {r['catalog_added']}\n"
                    f"─────────────────\n"
                    f"Всього ключів: {r['universal_keys']}\n"
                    f"Всього варіантів: {r['total_entries']}\n"
                    f"З кількома брендами: {r['multi_brand']}",
    ...
    @bot.message_handler(func=lambda m: m.text == '🔄 Схожі: повний rebuild')
    def btn_synonyms_full_rebuild(message):
        if message.chat.id != ADMIN_ID: return
        cmd_synonyms_full_rebuild(message)
        if message.chat.id != ADMIN_ID: return
    ...
    def kb_synonym(message):
        ...
        try:
            ...
            if count == 0:
                bot.reply_to(message, '📋 Кеш порожній.'); return
            with open(tmp, 'rb') as f:
                bot.send_document(
                    message.chat.id, f,
