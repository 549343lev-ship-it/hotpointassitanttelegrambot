"""handlers/text_handler.py — Прийом тексту: підказки, пошук, довільний текст."""
import re
import time
import threading
...
def register(bot, state: dict):
    ...
    def handle_text_search(message):
        ...
        _run_search(chat_id=message.chat.id, всі_позиції=позиції, items=[],
                    caption='', chosen_brand_map={},
                    msg_id=status.message_id, bot=bot, _state=state)

    # ── ручний ввід (_manual_wait) ────────────────────────────────────────────
    ...
    def handle_manual_input(message):
        ...
        if mode == 'new_client':
            ...
            _create_client(message.chat.id, query, message, bot)
    ...
    def handle_text_hint(message):
        ...
        # ── ПРІОРИТЕТ: waiting_hint — підказка після вибору клієнта ──────────
        ...
        # ── Схожий на список → питаємо що робити ─────────────────────────────
        is_list = bool(re.search(r'\d', text)) and (
            '\n' in text or ';' in text or len(text.split()) > 5)
        if is_list:
            state.setdefault('_pending_text', {})[chat_id] = {'text': text, 'ts': time.time()}
            bot.reply_to(message, "Це схоже на список товарів. Що робити?",
        ...
        def _expire():
            ...
            if hints.get(chat_id) == text:
                hints.pop(chat_id, None)
    ...
    def handle_text_choice(call):
        ...
        if call.data == "txts" and text:
            ...
            _run_search(chat_id=chat_id, всі_позиції=позиції, items=[],
                        caption='', chosen_brand_map={},
                        msg_id=status.message_id, bot=bot, _state=state)
