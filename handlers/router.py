...
TREE: list[tuple] = [
    ...
    ('kn.u.f.a', 'sewage',  [r'(коліно|муфта|трійник|заглушка|редукц|перехід|хрестовина).*(канал|°|\bф\d{2,3}).*asg(?!.*латун|.*нікел)'
                              r'|asg.*(коліно|трійник|муфта|заглушка|редукц|перехід).*(канал|°|\bф\d{2,3})'],
                            ['ASG', 'Фитинг']),

    ('kn.u.f.b', 'sewage',  [r'(коліно|муфта|трійник|заглушка|редукц|перехід|хрестовина).*(канал|°|\bф\d{2,3}).*ostendorf'
                              r'|ostendorf.*(коліно|трійник|муфта|заглушка|редукц|перехід).*(канал|°|\bф\d{2,3})'],
                            ['OSTENDORF', 'Ostendorf']),
    ...
    ('kn.u.p.b', 'sewage',  [r'труба.*ostendorf|ostendorf.*труба'
    ...
    ('ps.r.g',  'push_systems', [r'(муфта|коліно|трійник|фітинг).*raftec'
    ...
    # Фітинги PUSH без виробника → загальний ps (шукатиме по всіх виробниках через DEFAULT_BRAND_PRIORITY)
    # НЕ додаємо окремий запис — вони потраплять у ps через нижній рядок
    ...
    # Хром: подовжувач хром, ніпель хром, перехід хром → ar.c (хромовані)
    ...
    # Жовта латунь: raftec gold, lexline, перехідники латунь
