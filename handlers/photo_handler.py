...
def _ask_order_setup(message, item: dict, hint_already: bool,
    ...
    if chat_id in state.get('user_batches', {}) and state['user_batches'][chat_id].get('items'):
    ...
    def _flush(cid, msg):
        ...
        if cid in state.get('user_batches', {}) and state['user_batches'][cid].get('items'):
            for it in p['items']:
