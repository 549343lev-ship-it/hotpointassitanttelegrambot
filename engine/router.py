...
TEXT_RULES = [          # список (паттерн, категорія) — перший збіг перемагає
    ...
    # ─ PUSH / PEX (найвищий пріоритет) ─────────────────────────────────────
    (r'push|пуш|pex|пекс|натяжн|гільз|євроконус|rautitan|raubasic'
     r'|aquapex|heat.pex|general.fitting', 'push_systems'),
    ...
    # ─ НІПЕЛЬНІ КОЛІНА PPR (з різьбою РВ/РЗ/РН або маркером ВЗ) ────────────
    # РВ/РЗ/РН = коліно з різьбою (паяне + різьба) → завжди plastic_ppr
    # ВЗ у PPR = рівноосьове без різьби → теж plastic_ppr
    # НГ = настінне коліно для гіпсокартону → plastic_ppr
...
SUBCAT_RULES = [    # (regex на normalized+original, keywords для поля group в каталозі)
    ...
    (r'біметал|bi.vulcan|алюмін.*радіат',                      ['біметал', 'алюміній']),
    # Насоси
