...
TYPE_TO_CAT = {         # тип → основна категорія
    ...
    'ніпель':       'adapters_reducers',
    'штуцер':       'adapters_reducers',
    'подовжувач':   'adapters_reducers',
...
TEXT_RULES = [          # список (паттерн, категорія) — перший збіг перемагає
    ...
    (r'push|пуш|pex|пекс|натяжн|гільз', 'push_systems'),
    ...
    (r'перехідн|редукц|футорк|ніпел|штуцер|подовжувач|бочон|напівзгін'
     r'|згін\b|муфта\s+(перех|редукц)|муфта\s+вв|муфта\s+зз', 'adapters_reducers'),
    ...
    (r'радіатор|термоголовк|термостат\w*\s+клапан|клапан\s+радіатор'
     r'|комплект\s+підключ\w+\s+радіат|hidros|хідрос|idmar', 'radiators_radiatorsvalve'),
    ...
    (r'тепл\w+\s+підлог|теплопідлог|колектор\s+(тп|підлог)|терморегул\w+\s+(підлог|tp)'
     r'|монтажн\w+\s+стрічк|демпфер|якір\w+\s+скоб|такер', 'underfloor_heating'),

    # ─ Утеплювач ─
    (r'утеплюв|мірелон|мирелон|k.?flex|thermaflex|plm\b|плм\b|ізоляц\w+\s+труб', 'insulation'),

    # ─ Водоміри ─
    (r'лічильн|водомір|счетчик\s+воды|dn\s*(15|20|25)\s*лічильн', 'water_meters'),
    ...
    (r'хомут|скоб\w+\s+(монтаж|кріпл)|дюбель|шпилька|льон\b|тефлон|фум\b'
     r'|сантехн\w+\s+лен|пакля|прокладк|ущільн|розхідник', 'fasteners_sealants'),
