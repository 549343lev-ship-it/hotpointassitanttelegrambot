...
# ═══════════════════════════════════════════════════════════════════════════════
ADMIN_ID           = 395121797
PENDING_RULES_FILE = os.path.join(DATA_DIR, "pending_rules.json")
...
NOT_FOUND_FILE = os.path.join(DATA_DIR, "not_found_log.json")
...
# ═══════════════════════════════════════════════════════════════════════════════
def get_rules() -> str:
    """Повна база знань для Gemini (OCR, нормалізація). Читається з rules.txt."""
    if not os.path.exists(RULES_FILE):
        print("⚠️ rules.txt не знайдено — Gemini працює без бази знань", flush=True)
        return ""
    try:
        with open(RULES_FILE, encoding="utf-8") as f:
            return f.read().strip()
    except Exception as e:
        print(f"⚠️ get_rules: {e}", flush=True)
        return ""

def add_rule(rule: str):
    """Додає правило в кінець rules.txt."""
    try:
        content = ""
        if os.path.exists(RULES_FILE):
            with open(RULES_FILE, encoding="utf-8") as f:
                content = f.read()
        if "ДОДАНІ ПРАВИЛА" not in content:
            with open(RULES_FILE, "a", encoding="utf-8") as f:
                f.write("\n# ═══════════ ДОДАНІ ПРАВИЛА (команда: правило <текст>) ═══════════\n")
        with open(RULES_FILE, "a", encoding="utf-8") as f:
            f.write(f"- {rule}\n")
    except Exception as e:
        print(f"⚠️ add_rule: {e}", flush=True)
...
# ═══════════════════════════════════════════════════════════════════════════════
# ПРАВИЛА ВИБОРУ ДЛЯ CLAUDE — читаються з pick_rules.txt
# ═══════════════════════════════════════════════════════════════════════════════
PICK_RULES_FILE = os.path.join(DATA_DIR, "pick_rules.txt")

def get_pick_rules() -> str:
    """Компактні правила вибору для Claude + ручні правила менеджерів з rules.txt."""
    pick = ""
    if os.path.exists(PICK_RULES_FILE):
        try:
            with open(PICK_RULES_FILE, encoding="utf-8") as f:
                pick = f.read().strip()
        except Exception as e:
            print(f"⚠️ get_pick_rules: {e}", flush=True)
    else:
        print("⚠️ pick_rules.txt не знайдено — Claude вибирає без правил", flush=True)

    # Беремо тільки ручні правила менеджерів (після маркера ДОДАНІ ПРАВИЛА)
    extra = ""
    if os.path.exists(RULES_FILE):
        try:
            with open(RULES_FILE, encoding="utf-8") as f:
...
# Перевіряємо наявність файлів знань при старті
def _check_knowledge_files():
    missing = []
    if not os.path.exists(RULES_FILE):
        missing.append("rules.txt")
    if not os.path.exists(PICK_RULES_FILE):
        missing.append("pick_rules.txt")
    if missing:
        print(f"⚠️ Відсутні файли знань: {', '.join(missing)}", flush=True)
        print(f"   Завантаж на диск у {DATA_DIR}/", flush=True)
    else:
        r = len(open(RULES_FILE, encoding='utf-8').read().splitlines())
        p = len(open(PICK_RULES_FILE, encoding='utf-8').read().splitlines())
        print(f"📖 Знання завантажено: rules.txt ({r} рядків), pick_rules.txt ({p} рядків)", flush=True)

_check_knowledge_files()
...
КРОК 1 — РОЗПІЗНАЙ СИМВОЛИ І СХЕМИ (ПЕРЕД нормалізацією!):
Майстри часто малюють замість слів. Розпізнай ДО того як нормалізувати:
• Будь-який ⊥ або Т або перевернуте Т = ТРІЙНИК (навіть якщо намальовано від руки нечітко)
• Будь-який ∠ або L або кут з двох ліній = КОЛІНО
• Y-подібний або три лінії від точки = ТРІЙНИК (редукційний якщо розміри різні)
• Закреслений рядок (лінія через текст) = ПРОПУСТИТИ, не додавати
• -//- або «теж» = повтор типу з попереднього рядка (новий розмір/кількість)
• Рядок що починається з «х» або «ф» без назви = продовження попереднього типу
Приклади розпізнавання:
  ⊥110х110х50 → Трійник канал ф110х110х50 87, sewage
  ∠110х45 → Коліно канал ф110 45, sewage
  Т25х16х25 → Трійник PPR ф25х16х25, plastic_ppr
  L20 (у PPR-контексті) → Коліно PPR ф20 90, plastic_ppr
  ~~Труба 25 50м~~ → ПРОПУСТИТИ

КРОК 2 — КАТЕГОРІЯ (критично!):
• Є слово PPR/ПП/пайка + фітинг → plastic_ppr
• Діаметри 32/40/50/75/110 + каналізаційний фітинг/труба → sewage
• Є слово push/пуш/натяжн/PEX → push_systems
• Кран/вентиль/засувка → shutoff_valves
• Перехідник/ніпель/футорка/муфта різьб БЕЗ слова PPR → adapters_reducers
• Підводка/шланг до змішувача/унітаза → hoses
• Фільтр грубої/косий/грязьовик/під пломбу → filtration
• Насос/циркуляційний → pumps
• Радіатор/термоголовка/клапан радіатор → radiators_radiatorsvalve
• Колектор ТП/євроконус/скоба/демпфер → underfloor_heating
• Утеплювач/ізоляція труби PLM → insulation
• Металопластик/м/п/MP → metal_plastic
• Запобіжний/зворотний клапан/манометр/редуктор → safety_valves
• Бойлер/водонагрівач → water_heaters
• Котел → boilers
• Сифон/гофра унітаз/трап/інсталяція → siphons_fittings
• Хомут/клипса/пакля/фум/паста/дюбель → fasteners_sealants
• Рушникосушарка/ТЕН для рушника → towel_warmers
• Змішувач/картридж змішувача → mixers

КРОК 3 — НОРМАЛІЗУЙ (КОРОТКО, без зайвих слів і PN):
JSON масив ТІЛЬКИ:
[{{"original":"що написано/намальовано","normalized":"коротка назва","qty":"кількість",
"category":"категорія","type":"труба/коліно/трійник/муфта/кран/гільза/перехід/...",
"dia":[110,50],"angle":87,"thread":"1/2 або null"}}]
type=тип виробу ОДНИМ словом; dia=ВСІ діаметри числами; angle=кут або null; thread=різьба або null.
Виробника пиши ТІЛЬКИ якщо він у підказці або в рядку. НЕ вигадуй PN/SDR/товщину стінки."""
...
КРОК 1 — СИМВОЛИ: ⊥/Т=трійник | ∠/L=коліно | ~~текст~~=пропустити | -//-=повтор типу
КРОК 2 — КАТЕГОРІЯ: є PPR/ПП→plastic_ppr | 32/40/50/110 канал→sewage | push/пуш→push_systems |
  кран/вентиль→shutoff_valves | перехідник/ніпель БЕЗ PPR→adapters_reducers |
  підводка/шланг→hoses | фільтр косий→filtration | насос→pumps | радіатор→radiators_radiatorsvalve |
  утеплювач труби→insulation | хомут/пакля/фум→fasteners_sealants
КРОК 3 — НОРМАЛІЗУЙ кожну позицію (КОРОТКО!).
...
Виробника пиши ТІЛЬКИ якщо він у підказці або в запиті."""
    try:
