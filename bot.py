...
# Кожна позиція: {"normalized": "Труба ПВХ каналізаційна 50мм 1м", "qty": "2", "original": "тdef нормалізувати_фото(image_b64, caption=""):
 rules = get_rules()
 rules_block = f"\nДодаткові правила від користувача:\n{rules}" if rules else ""
 prompt = f"""Ти — експерт із сантехніки. На фото рукописний список замовлення від майстраЗАВДАННЯ:
...
[
 {{"original": "що написано на фото", "normalized": "Стандартна торгова назва", "qty": "кіль]"""
 resp = client.messages.create(
 model="claude-sonnet-4-5",
 max_tokens=2048,
 messages=[{"role": "user", "content": [
 {"type": "image", "source": {"type": "base64", "media_type": "image/jpeg", "data" {"type": "text", "text": prompt}
 ]}]
 )
...
[
 {{"original": "оригінальний рядок", "normalized": "Стандартна торгова назва", "qty": "кільк]"""
 resp = client.messages.create(
 model="claude-sonnet-4-5",
 max_tokens=2048,
...
def process_batch(chat_id):
 ...
 if not всі_позиції:
 bot.edit_message_text(" Не вдалося розпізнати жодної позиції.", chat_id=chat_id, me return
 ...
 if not_found:
 звіт += f"\n Не знайдено ({len(not_found)} шт.) — дивись лист 'Не знайдено' в Excel звіт += "\n".join(f"• {n}" for n in not_found[:5])
 if len(not_found) > 5:
...
def handle_text(message):
 ...
 bot.reply_to(message, "Напиши запит після слова 'пошук'.\nНаприклад: `пошук труба 50`bot.polling(none_stop=True)
