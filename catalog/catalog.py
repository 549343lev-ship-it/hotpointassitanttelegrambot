...
def tokenize(text: str) -> set:     # перетворює назву на набір токенів (нормалізує розміри, різьби, кути)
    ...
    def encode_length(m):   # кодує дробову довжину в сантиметри щоб розрізняти 0.5м→50 і 3.0м→300
        val = float(m.group(1).replace(',', '.'))
        cm  = int(round(val * 100))
        return f" {cm} "    # просто ціле число в сантиметрах — токенізатор підхопить його

    t = re.sub(r'l\s*[=:]?\s*(\d+[.,]\d+)\s*м?', encode_length, t)
